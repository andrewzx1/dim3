...
void project_palette_fill_rings(void)
{
	...
	for (n=0;n!=iface.ring_list.nring;n++) {
		list_palette_add_string_selectable_button(&project_palette,(kRingProperyName+n),list_button_minus,(kRingProperyDelete+n),iface.ring_list.rings[n].name,(state.proj.cur_ring_idx==n),FALSE,FALSE);
	}
