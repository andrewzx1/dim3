...
void property_palette_fill_sounds(void)
{
	...
	for (n=0;n!=iface.sound_list.nsound;n++) {
		list_palette_add_string_selectable_button(&property_palette,(kSoundProperyName+n),list_button_minus,(kSoundProperyDelete+n),iface.sound_list.sounds[n].name,(state.cur_sound_idx==n),FALSE);
	}
