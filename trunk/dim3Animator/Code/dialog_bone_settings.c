			...
			switch (cmd.commandID) {
				...
				case kHICommandOK:
					...
					if (tag==model_null_tag) {
						dialog_alert("Bad Bone Tag","Bones require a tag to uniquely identify them.  ???? is the null tag and is not allowed.  Please pick a proper 4 charater tag.",NULL,NULL);
						return(noErr);
					}
					...
					if (model_check_bone_duplicate_tag(&model,bone)) {
						...
						dialog_alert("Bad Bone Tag","Bones require a tag to uniquely identify them.  This tag is already being used by another bone, choose another 4 character tag.",NULL,NULL);
						return(noErr);
					}
					...
					if (model_check_bone_circular(&model,bone)) {
						...
						dialog_alert("Bad Bone Parenting","The parent of this bone will make a set of bones with circular references.  Please change the parent tag.",NULL,NULL);
						return(noErr);
					}
