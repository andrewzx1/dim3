...
void shadow_render_model_mesh_2(model_type *mdl,int model_mesh_idx,model_draw *draw)
{
	...
	d3pnt						*hpt,bound_min,bound_max,light_pnt;
...
//	if (map_poly_count==0) return;
	...
	for (n=0;n!=model_mesh->nvertex;n++) {
		...
		ept->x=spt->x+(int)(vct.x*f_light_intensity);
		ept->x=spt->x+(int)(vct.y*f_light_intensity);
		ept->x=spt->x+(int)(vct.z*f_light_intensity);
	...
	ray_trace_map_by_point_array(model_mesh->nvertex,shadow_spt,shadow_ept,shadow_hpt,shadow_hit,&base_contact,shadow_contact);
