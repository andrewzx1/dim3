...
void crosshair_draw(obj_type *obj,weapon_type *weap)
{
	...
	if ((weap->crosshair.type==ct_center) && (camera.setup.mode!=cv_fpp)) return;
	...
	view_primitive_2D_texture_quad(crosshair_draw->gl_id,&crosshair_draw->color,1.0f,lft,rgt,top,bot,0.0f,1.0f,0.0f,1.0f);
}
