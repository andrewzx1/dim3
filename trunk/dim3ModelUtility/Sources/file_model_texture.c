	...
	for (i=0;i!=max_model_texture;i++) {
        ...
        for (k=0;k!=max_texture_frame;k++) {
			...
			if (frame->name[0]!=0x0) {
				...
				bitmap_open(&frame->bitmap,path,modelutility_settings.anisotropic_mode,modelutility_settings.mipmap_mode,texture->pixelated,FALSE);

					// bumpmap

				sprintf(name,"%s_n",frame->name);
				file_paths_data(&modelutility_settings.file_path_setup,path,sub_path,name,"png");
				bitmap_open(&frame->bumpmap,path,modelutility_settings.anisotropic_mode,modelutility_settings.mipmap_mode,texture->pixelated,FALSE);
				
					// specular map

				sprintf(name,"%s_s",frame->name);
				file_paths_data(&modelutility_settings.file_path_setup,path,sub_path,name,"png");
				bitmap_open(&frame->specularmap,path,modelutility_settings.anisotropic_mode,modelutility_settings.mipmap_mode,texture->pixelated,FALSE);

					// glow map

				sprintf(name,"%s_g",frame->name);
				file_paths_data(&modelutility_settings.file_path_setup,path,sub_path,name,"png");
				bitmap_open(&frame->glowmap,path,modelutility_settings.anisotropic_mode,modelutility_settings.mipmap_mode,texture->pixelated,TRUE);
			}
