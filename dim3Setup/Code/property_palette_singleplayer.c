...
void property_palette_fill_singleplayer(void)
{
	int					n;

	list_palette_set_title(&property_palette,"Singleplayer",NULL,NULL,NULL,NULL,NULL);

		// settings

	list_palette_add_header(&property_palette,0,"Settings");
	list_palette_add_checkbox(&property_palette,kSPPropertySkill,"Skill",&iface.singleplayer.skill,FALSE);
	list_palette_add_checkbox(&property_palette,kSPPropertyMapPick,"Map Pick",&iface.singleplayer.map_pick,FALSE);

		// options

	list_palette_add_header_button(&property_palette,kSPPropertyOptionAdd,"Options",list_button_plus);

	list_palette_sort_mark_start(&property_palette);
	
	for (n=0;n!=iface.singleplayer.option_list.noption;n++) {
		list_palette_add_string_selectable_button(&property_palette,(kSPPropertyOptionName+n),list_button_minus,(kSPPropertyOptionDelete+n),iface.singleplayer.option_list.options[n].name,(state.cur_singleplayer_option_idx==n),FALSE);
	}
...
void property_palette_click_singleplayer(bool double_click)
{
	int					id,idx,sz;
	...
	if ((id>=kSPPropertyOptionName) && (id<(kSPPropertyOptionName+max_net_option))) {
		state.cur_singleplayer_option_idx=id-kSPPropertyOptionName;
	...
	if (id==kSPPropertyOptionAdd) {
		...
		state.cur_singleplayer_option_idx=idx;
