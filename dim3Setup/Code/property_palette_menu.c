...
void property_palette_fill_menu(void)
{
	...
	for (n=0;n!=iface.menu_list.nmenu;n++) {
		list_palette_add_string_selectable_button(&property_palette,(kMenuPropertyMenuName+n),list_button_minus,(kMenuPropertyMenuDelete+n),iface.menu_list.menus[n].name,NULL,(state.cur_menu_idx==n),FALSE);
	}
...
void property_palette_click_menu(int id)
{
	...
	if (id==kMenuPropertyMenuAdd) {
		...
		if (iface.menu_list.nmenu>=max_iface_menu) {
			os_dialog_alert("Reached the maximum number of menus","Add Menu");
			return;
		}
