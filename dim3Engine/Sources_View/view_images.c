...
int view_images_load_single(char *path,bool simple)
{
	...
	if (simple) {
		bitmap_open(&image->bitmap,path,anisotropic_mode_none,mipmap_mode_none,FALSE,FALSE);
	}
	else {
		bitmap_open(&image->bitmap,path,setup.anisotropic_mode,setup.mipmap_mode,FALSE,FALSE);
	}
