		...
		case kEventProcessCommand:
			...
			switch (cmd.commandID) {
				...
				case kMapGroupAddButton:
					if (map.ngroup>=max_group) {
						os_dialog_alert("Can Not Create Group","You've reached the maximum number of groups.");
						return(noErr);
					}
				...
				case kMapGroupDeleteButton:
					...
					for (n=0;n!=map.nmovement;n++) {
						if ((map.movements[n].group_idx==dialog_map_groups_current_idx) || (map.movements[n].reverse_group_idx==dialog_map_groups_current_idx)) {
							os_dialog_alert("Can Not Delete Group","This group is used in a group movement, you will need to delete or edit the group movement first.");
							return(noErr);
						}
					}
