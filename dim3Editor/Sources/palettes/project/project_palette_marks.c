...
void project_palette_fill_marks(void)
{
	...
	for (n=0;n!=iface.mark_list.nmark;n++) {
		list_palette_add_string_selectable_button(&project_palette,(kMarkProperyName+n),list_button_minus,(kMarkProperyDelete+n),iface.mark_list.marks[n].name,(state.proj.cur_mark_idx==n),FALSE,FALSE);
	}
