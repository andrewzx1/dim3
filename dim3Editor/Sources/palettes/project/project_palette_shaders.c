...
void project_palette_fill_shaders(void)
{
	...
	for (n=0;n!=iface.shader_list.nshader;n++) {
		list_palette_add_string_selectable_button(&project_palette,(kShaderProperyName+n),list_button_minus,(kShaderProperyDelete+n),iface.shader_list.shaders[n].name,(state.proj.cur_shader_idx==n),FALSE,FALSE);
	}
